...
import { useCategoryBySlug, useProductsByCategory, useCategories } from "@/hooks/use-products";
...
export default function CategoryPage() {
  ...
  const { data: allProducts = [] } = useProductsByCategory(category?.id || "");
    ...
    <div className="min-h-screen py-6">
      <div className="container">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <h1 className="text-2xl font-bold text-foreground md:text-3xl">{category.icon} {category.name}</h1>
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mb-4">
                ...
                <div className="space-y-1.5">
                  {([["all", "الكل"], ["in-stock", "متوفر"], ["out-of-stock", "نفذ"]] as [FilterStock, string][]).map(([value, label]) => (
                    <button key={value} onClick={() => setStockFilter(value)}
                      className={`block w-full rounded-lg px-3 py-1.5 text-right text-xs transition-colors ${stockFilter === value ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-muted"}`}
                    >{label}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="space-y-1.5">
                  {([["all", "الكل"], ["monthly", "شهري"], ["3months", "3 أشهر"], ["yearly", "سنوي"]] as [FilterDuration, string][]).map(([value, label]) => (
                    <button key={value} onClick={() => setDurationFilter(value)}
                      className={`block w-full rounded-lg px-3 py-1.5 text-right text-xs transition-colors ${durationFilter === value ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-muted"}`}
                    >{label}</button>
                  ))}
                </div>
              </div>
